...
(async()=>{
  ...
  const home=async()=>{await p.evaluate(()=>{try{nccChiudiPopup();}catch(e){}try{closeAdd();}catch(e){}try{['scnOv','popOv'].forEach(i=>{const x=document.getElementById(i);if(x)x.remove();});}catch(e){}goHome();});await p.waitForTimeout(700);};
  ...
  console.log('righe Topografia:',righe.join(' | '));
  for(const nome of righe){
    ...
    if(va[va.length-1]==='pagina:topo'&&!/Correggi/.test(nome))fails.push(nome+': non apre niente');
