/* GIRO DELLE PIAZZE, fotogramma per fotogramma, a tempo reale.
   Per ogni riga della pagina Piazze: tocco, filmo cosa c'e' al centro dello schermo ogni ~30 ms,
...
(async()=>{
  ...
  const home=async()=>{await p.evaluate(()=>{try{nccChiudiPopup();}catch(e){}try{['scnOv','popOv','pzOv','pzMapOv','scOv','pzqOv','pzGeoP'].forEach(i=>{const x=document.getElementById(i);if(x)x.remove();});document.body.classList.remove('pz-aperto');}catch(e){}goHome();});await p.waitForTimeout(700);};
  await home();await p.evaluate(()=>nccSez('pz'));await p.waitForTimeout(800);
  const righe=await p.evaluate(()=>[...document.querySelectorAll('#scnOv .qc-riga')].map(r=>r.querySelector('.qc-lt b').textContent.trim()));
  console.log('righe Piazze:',righe.join(' | '));
  for(const nome of righe){
    ...
    await p.waitForTimeout(2600);   /* come una persona: resto un po' sulla schermata prima di tornare */
    ...
    await new Promise(r=>setTimeout(r,120));let torna=await film(1100);
    if(/Mappa a ragno/.test(nome)&&torna[torna.length-1]==='pzMapOv'){await p.evaluate(()=>document.querySelector('#pzMapOv .pzm-home').click());torna=torna.concat(await film(900));}
    if(/Verifica/.test(nome)&&torna[torna.length-1]==='pzOv'){await p.evaluate(()=>document.querySelector('#pzOv .pz-hd2 .pz-x').click());torna=torna.concat(await film(900));}
    ...
    if(va[va.length-1]==='pagina:pz')fails.push(nome+': non apre niente');
    if(torna.some(x=>/^vuoto/.test(x)||x==='homeScreen'))fails.push(nome+': schermo vuoto o Home di passaggio tornando ('+torna.join(' → ')+')');
    if(fine!=='pagina:pz')fails.push(nome+': indietro non torna alla pagina Piazze ma a '+fine);
  }
